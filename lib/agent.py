    ...
    def train(self, config, logdir, resume=True):
        ...
        for episode in range(start_episode, max_episode):
            ...
            for t in range(config.env.T_max):
                ...
                next_state, reward, done = env.step(action, discrete=False)
                ...
                if not done:
                    last_value = info['net'](next_state)[-1]
    ...
    def eval(self, config, logdir, ckpt_num=None, writer=None):
        ...
        for t in range(config.env.T_max):
            with torch.no_grad():
                ...
                actions.append(torch.from_numpy(action))
                _, reward, _ = env.step(action, discrete=False)
...
class A2C_DISCRETE_AGENT:
    ...
    def train(self, config, logdir, resume=True):
        ...
        device = config.device
        net = A2C_DISCRETE_MODEL(config, state_size=6, n_actions=4).to(device)
        if config.multi_gpu:
            net = nn.DataParallel(net)
        optimizer = optim.Adam(net.parameters(), lr=config.train.lr)
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=config.train.max_episode)
        ...
        for episode in range(start_episode, max_episode):
            ...
            for t in range(config.env.T_max):
                ...
                logit, _ = info['net'](state)
                action = torch.max(logit, dim=1)[1].cpu().numpy()
                actions.append(torch.from_numpy(action))

                next_state, reward, done = env.step(action, discrete=True)
                next_state = torch.from_numpy(next_state).type(torch.float32).to(device)
                reward = torch.from_numpy(reward).type(torch.float32).to(device)
                if not done:
                    last_value = info['net'](next_state)[-1]
            ...
            logits, values = info['net'](states)
            loss_value = F.mse_loss(values.squeeze(-1), ref_values)

            advantage = ref_values.unsqueeze(-1) - values.detach()
            log_prob = F.log_softmax(logits, dim=1)
            log_prob = advantage * log_prob.gather(1, actions.unsqueeze(-1))
            loss_policy = -log_prob.mean()
    ...
    def eval(self, config, logdir, ckpt_num=None, writer=None):
        ...
        for t in range(config.env.T_max):
            with torch.no_grad():
                ...
                actions.append(torch.from_numpy(action))
                actions_real.append(torch.from_numpy(env.discrete_actions[list(action), :]))
                _, reward, _ = env.step(action, discrete=True)
                rewards.append(torch.from_numpy(reward).unsqueeze(1))

        states = torch.cat(states, dim=0).cpu().numpy()
        actions = torch.cat(actions_real, dim=0).cpu().numpy()
        rewards = torch.cat(rewards, dim=0).cpu().numpy()
        cum_reward = rewards.sum() * env.scaler
        writer.add_scalar('cum_reward', cum_reward, info['episode'])
